...
class SupabaseCRUD:
    ...
    def get_rows_by_field(self, table_name: str, field: str, value: Any):
        """Retrieves all rows matching a specific field value."""
        response = self.supabase_client.table(table_name).select('*').eq(field, value).execute()
        return AttrDict.convert(response.data)
